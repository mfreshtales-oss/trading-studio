...
Loads {SYMBOL}_1H.csv, runs instruments/{SYMBOL}/strategy.generate_signals(),
simulates trades, and prints a JSON metrics dict to stdout.
...
Usage:
    ...
    python backtest.py --symbol US30
    python backtest.py --symbol NAS100
    python backtest.py --symbol SPX
...
def load_strategy(symbol: str):
    """Dynamically load instruments/{symbol}/strategy.py."""
    strategy_path = RESEARCH_DIR / "instruments" / symbol / "strategy.py"
    if not strategy_path.exists():
        raise FileNotFoundError(f"Strategy not found: {strategy_path}")
    spec = importlib.util.spec_from_file_location(f"strategy_{symbol}", strategy_path)
...
def run_backtest(df: pd.DataFrame, strategy) -> dict:
    ...
    if (df_sig["signal"] == 0).all():
        return {
            "total_trades": 0, "win_rate": 0.0, "profit_factor": 0.0,
            "total_r": 0.0, "max_drawdown_r": 0.0, "sharpe_r": 0.0,
            "wins": 0, "losses": 0, "note": "No signals generated",
        }
...
def main() -> None:
    ...
    parser.add_argument(
        "--symbol",
        default="XAUUSD",
        choices=["XAUUSD", "US30", "NAS100", "SPX", "BTCUSD"],
        help="Instrument to backtest (default: XAUUSD)",
    )
    parser.add_argument(
        "--data",
        default=None,
        help="Override CSV path (default: data/{SYMBOL}_1H.csv)",
    )
    args = parser.parse_args()

    data_path = Path(args.data) if args.data else RESEARCH_DIR / "data" / f"{args.symbol}_1H.csv"

    if not data_path.exists():
        print(
            json.dumps({"error": f"Data not found: {data_path}. Run: python fetch_data.py --symbol {args.symbol}"}),
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    try:
        strategy = load_strategy(args.symbol)
    except Exception as exc:
        print(json.dumps({"error": f"Failed to load strategy: {exc}"}), file=sys.stderr)
        sys.exit(1)
