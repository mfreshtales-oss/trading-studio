"""
strategy.py — US30 (Dow Jones) scalping strategy.
THIS FILE IS MODIFIED BY THE AGENT.
"""

import pandas as pd

# Hypothesis: US30 trends well on 1H; EMA 9/21 with wider stops to handle index volatility
STRATEGY_PARAMS: dict = {
    "fast_ema": 9,
    "slow_ema": 21,

    "momentum_lookback": 5,
    "momentum_threshold": 0.002,   # US30 needs bigger moves to confirm; ~80 pts on $40k
    ...
    "rr_ratio": 5.0,               # must remain >= 5.0
    "risk_dollars": 50.0,          # 50-point stop on US30 (~0.125% of price)
...
def generate_signals(df: pd.DataFrame) -> pd.DataFrame:
    """Generate entry signals. Returns df with 'signal' column (1=long, -1=short, 0=none)."""
    p = STRATEGY_PARAMS
    df = df.copy()
